...
from bumblebee.notifications.api.serializers.buzz_notif_serializers import (
    CommentBuzzNotificationSerializer,
    DownvoteBuzzNotificationSerializer,
    RebuzzBuzzNotificationSerializer,
    UpvoteBuzzNotificationSerializer,
)
from bumblebee.notifications.api.serializers.comment_notif_serializers import (
    DownvoteCommentNotificationSerializer,
    ReplyCommentNotificationSerializer,
    UpvoteCommentNotificationSerializer,
)
...
from bumblebee.notifications.api.serializers.rebuzz_notif_serializers import (
    CommentRebuzzNotificationSerializer,
    DownvoteRebuzzNotificationSerializer,
    UpvoteRebuzzNotificationSerializer,
)
from bumblebee.notifications.api.serializers.user_serializers import (
    NotificationOwnerSerializer,
)
from bumblebee.notifications.utils import get_notifications_for_userid
...
class UserGroupedNotificationView(APIView):
    """ """
    ...
    def _get_notifications(self, *args, **kwargs):
        ...
        return get_notifications_for_userid(self.request.user.id)
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            # buzz notifications
            buzz_upvote_notif_serialzier = UpvoteBuzzNotificationSerializer(
                notification_instances["buzz_notification"], many=True
            )
            buzz_downvote_notif_serialzier = DownvoteBuzzNotificationSerializer(
                notification_instances["buzz_notification"], many=True
            )
            buzz_comment_notif_serialzier = CommentBuzzNotificationSerializer(
                notification_instances["buzz_notification"], many=True
            )
            buzz_rebuzz_notif_serialzier = RebuzzBuzzNotificationSerializer(
                notification_instances["buzz_notification"], many=True
            )

            # rebuzz notification
            rebuzz_upvote_notif_serialzier = UpvoteRebuzzNotificationSerializer(
                notification_instances["rebuzz_notification"], many=True
            )
            rebuzz_downvote_notif_serialzier = DownvoteRebuzzNotificationSerializer(
                notification_instances["rebuzz_notification"], many=True
            )
            rebuzz_comment_notif_serialzier = CommentRebuzzNotificationSerializer(
                notification_instances["rebuzz_notification"], many=True
            )

            # comment notifications
            comment_upvote_notif_serialzier = UpvoteCommentNotificationSerializer(
                notification_instances["comment_notification"], many=True
            )
            comment_downvote_notif_serialzier = DownvoteCommentNotificationSerializer(
                notification_instances["comment_notification"], many=True
            )
            comment_reply_notif_serialzier = ReplyCommentNotificationSerializer(
                notification_instances["comment_notification"], many=True
            )

            # connection notifications
            new_follower_notif_serialzier = NewFollowerNotificationSerializer(
                notification_instances["follower_notification"], many=True
            )
            new_follower_request_notif_serialzier = (
                NewFollowerRequestNotificationSerializer(
                    notification_instances["follower_request_notification"],
                    many=True,
                )
            )
...
class UserGroupedNotificationListView(APIView):
    """ """
